...
function hintClicked(elHint) {
    if (gIsHintOn) return;
    if ((gIsGameOn) && (gHintsLeft !== 0) && (!elHint.classList.contains('clicked'))) {
...
function userInputMinePos(i) {
    var mineI = +prompt(`Please Enter The Row Number (0-3) Mine Number ${i + 1}`);
    while (mineI < 0 || mineI > gLevel.SIZE - 1) {
        alert('Please Enter A Valid Number.');
        var mineI = +prompt(`Please Enter The Row Number (0-3) Mine Number ${i + 1}`);
    }
    var mineJ = +prompt(`Please Enter The Column Number (0-3) Mine Number ${i + 1}`)
...
function alignHints() {
    ...
    switch (gHintsLeft) {
        case 1:
            ...
            elHint2.style.left = `${314}px`;
            break;
        case 2:
            elHint1.style.display = 'block';
            elHint2.style.display = 'block';
            elHint3.style.display = 'none';
            elHint1.style.left = `${269}px`;
            elHint2.style.left = `${351}px`;
            break;
        case 3:
            elHint1.style.display = 'block';
            elHint2.style.display = 'block';
            elHint3.style.display = 'block';
            elHint1.style.left = `${235}px`;
            elHint2.style.left = `${312}px`;
            elHint3.style.left = `${388}px`;
    }
}
...
function alignEyes() {
    ...
    switch (gEyesLeft) {
        case 1:
            ...
            elEye2.style.left = `${299}px`;
            break;
        case 2:
            elEye1.style.display = 'block';
            elEye2.style.display = 'block';
            elEye3.style.display = 'none';
            elEye1.style.left = `${253}px`;
            elEye2.style.left = `${338}px`;
            break;
        case 3:
            elEye1.style.display = 'block';
            elEye2.style.display = 'block';
            elEye3.style.display = 'block';
            elEye1.style.left = `${220}px`;
            elEye2.style.left = `${298}px`;
            elEye3.style.left = `${374}px`;
    }
}
